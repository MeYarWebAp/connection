...
with st.expander(""):
    st.markdown("<h5 style='text-align: center;color:lightblue'>معیار، ماشین جستجوگری هوشمند است که به طورخودکار، ۲۴ ساعته و هر ثانیه، بدون نیاز به کمک هیچ واسطه ای غیر از خود شما، به سلیقه ها و انتخاب های شما به طور دقیق گوش فرا می دهد و برای  شکل گیری خانواده ای جدید،  لینک هایی که معیارهای شما را دارا هستند به شما می شناساند. معیار، این کار را بر اساس درجه سخت گیری ای که خود شما تعیین می کنید، انجام می دهد. طرح معیار، هدف اقتصادی ندارد و ماموریتی بیشتر از لینک کردن اولیه برای خود تعریف نکرده است. معیار، پس از معرفی، فرض را برقدرت انتخاب، اعتماد و هوشمندی کاربران می گذارد</h5>", unsafe_allow_html=True )
